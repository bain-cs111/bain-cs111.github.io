...
You are going to try and create a row of circles of a fixed size using `iterated-beside`, which is a function that takes as input another function and a count and then displays an image defined by the inputted function `count` times (e.g. if you passed in a function call to `circle` and a `5` it would place 5 identical circles next to each other).
...
If you're in class make sure to check-in with your PM. You don't need to submit a `rkt` file.
