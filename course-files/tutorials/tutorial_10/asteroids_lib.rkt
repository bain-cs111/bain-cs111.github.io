...
(require mzlib/pconvert
         ...
         (only-in "./define_super_struct.rkt"
                  ...
                  define/linking mutable-variable-pack struct-spec-pack/defaults))
...
(provide
 ...
 (rename-out
  ...
  ;; for the tests to report errors; the autograder disables the error form
  [error asteroids-report-error])
...
(define-struct-name-specification homing-missile
  #:super missile
  #:fields ()
...
(define/linking (link-and-start-asteroids-game)
  #:links ((mutable-variable-pack all-game-objects)
           ...
           (struct-spec-pack/defaults homing-missile)
  ...
  (define (on-key-press key)
    (cond [(equal? key "up")
          ...
          [(equal? key "s")
           (cond
             [(procedure? make-homing-missile)
              (fire-missile! make-homing-missile)]
             [else
              (printf "Asteroids: warning: the 's' key is pressed but ")
              (printf "the homing-missile structure is not defined.\n")])]
          [else
           (void)]))
  ...
  ;; use make-missile or make-homing-missile
