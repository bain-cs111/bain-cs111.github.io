...
(define (check-and-extract-method dispatch-info self-arg)
  ...
  (cond
    [(has-method? self-arg) (method-ref self-arg)]
     ...
     (when (or (not (attribute super-name))
       ...
       (raise-syntax-error 'define-struct
                           (format (string-append
                                    ...
                                    "the super struct ~a\n  given: ~a")
                                   spec-super-name
                                   (syntax-e #'(~? super-name "no super struct")))
                           struct-stx
                           #'name-or-super-form))]
    [else
     ...
     (raise-syntax-error
      ...
      (format
       (string-append
        ...
        "name after define-struct, but found something else: ~a")
       ...
       (for-each
        (lambda (field)
          (unless (identifier? field)
            (raise-syntax-error
             'define-struct
             (format "expected a field name, but found something else: ~a"
                     (syntax->datum field))
             stx
             field))
         ...
         (filter-map
          (lambda (a-method-stx)
            (syntax-parse a-method-stx
              ...
              [(asl:define . (~or ((method-name:id) . body-exprs)
                                  (method-name:id (asl:lambda () . body-exprs))
                                  (method-name:id (asl:λ () . body-exprs))))
               (raise-syntax-error
                'define-struct
                (format
                 (string-append
               ...
               (raise-syntax-error
                ...
                (format
                 (string-append
                  ...
                  "after the #:methods keyword, but found something else: ~a")
...
(define-for-syntax (parse-struct-method-name a-method-stx)
  (syntax-parse a-method-stx
    #:literals (asl:define asl:lambda asl:λ)
    [(~or (asl:define (method-name:id . rest-args-ids) . rest-exprs)
          (asl:define method-name:id . rest-exprs))
     #'method-name]))
...
(define-syntax (define-struct/methods stx)
  (syntax-parse stx
    [(_ name-or-super fields . rest)
     (define methods-stxs
       (check-and-extract-define-struct-methods stx))
     ;; At this point, check-and-extract-define-struct-methods guarantees
       ...
       [methods-stxs
        ...
        (with-syntax ([(method-name ...) method-names-stxs])
          #`(begin
              ...
              #,(quasisyntax/loc stx
                  (define-struct/methods-impl name-or-super fields . rest))))]
       [else
        (syntax/loc stx
  ...
  (cond
    [methods-stxs
     ...
     (define method-names-stxs
       (for/list ([a-method-stx (in-list methods-stxs)])
         (parse-struct-method-name a-method-stx)))
     (define-values (local-method-names-stxs method-implementations-stxs)
       (for/lists (local-method-names method-impls)
                  ([a-method-stx (in-list methods-stxs)])
         (parse-and-transform-struct-method spec-info/#f a-method-stx)))
     (with-syntax ([(method-implementation ...) method-implementations-stxs])
       #`(begin
