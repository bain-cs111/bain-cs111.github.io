...
(define-struct student (netid fave-class))

; Constructor - to make a student
; make-student: string string number -> student
(define connor
  (make-student "abc1234" "COMP_SCI 111"))

; Accessors - to get specific attributes of a student
; student-netid: student -> string
(student-netid connor)

; student-fave-class: student -> string
(student-fave-class connor)
(check-expect (student-fave-class connor)
              "COMP_SCI 111")

; Predicate - tells you if something is that type!
; student?: student -> boolean
(student? connor)
(check-expect (student? connor) #true)

; a snake is:
; - (make-snake number string)
;     it's defined by its weight and its favorite food
