...
; add-v2: employee database -> database
; adds the person to the database, maintaining ascending order (note this is challenging)
(define (add-v2 per db)
  (local [(define (add-helper per db new-db)
            (if (empty? db)
                (cons per new-db) ; if we're at the end or empty, add the new person
                (if (< (employee-ssn per)
                       (employee-ssn (first db))) ; if the new ssn is greater, keep going
                    (append (reverse new-db) ; cause cons builds "backwards..."
                            (cons per db))
                    (add-helper per
                                (rest db)
                                (cons (first db)
...
(check-expect (add-v2 george-e (list ava-e jessica-e steve-e))
              (list ava-e george-e jessica-e steve-e))
(check-expect (add-v2 ava-e empty)
              (list ava-e))

; lookup-v2: number database -> person or false
...
(define (lookup-v3 n db)
  (cond [(empty? db)         false] ; what do you do if the db is empty?
