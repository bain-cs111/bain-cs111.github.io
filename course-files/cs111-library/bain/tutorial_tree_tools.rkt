...
(define/implicit-parameter (node->pict p)
  ...
  (define node-text
    (above
     ...
     (text label 11 "magenta")))
  (define radius
    (quotient (+ 14 (max (image-width node-text) (image-height node-text)))
              2))
  (overlay node-text
...
(define/implicit-parameter (draw-binary-tree T)
  ...
  (let ([who 'draw-binary-tree])
    (check-binary-tree T))
  (pict->2htdp (do-draw-binary-tree T)))

(define/implicit-parameter (do-draw-binary-tree T)
  #:freevars ()
  (naive-layered
    (tree->layout T
                  #:node? branch?
