...
#|
;; These are auxiliary code for producing figures in Exercise 5.
;; They are not part of the solution. Run write-images from exercise_5_solution
...
(define/implicit-parameter (write-images)
  #:freevars (joffrey sara russ connor make-node)
  ...
  (save-image (draw-ancestry-tree joffrey) "joffrey.png")
  (save-image (draw-bst
               (make-node sara
                          #false
                          (make-node russ
                                     #false
                                     (make-node connor #false #false))))
              "the_bsts_1.png")
  (save-image (draw-bst
               (make-node connor
                          (make-node sara
                                     #false
                                     (make-node russ #false #false))
                          #false))
              "the_bsts_2.png")
  (void))
; |#
